...
async function processData(data, boardId) {
  ...
  for (const record of data) {
    ...
    const s = JSON.stringify(`{"numeric_mkw8251h": "${SI_No_Col_Width}"},{"numeric_mkw8g4na": "${SI_No_Col_Depth}"},{"status": "${subitemStatus}"},{"numeric_mkw8k2py": "${SI_No_Col_Value}"}`);
    await createSubitemMutation(itemIdMap[Item], s, subitemName);
  }
  console.log(itemIdMap, groupIdMap, "ASDASASDASDSD");
}
async function parseCsvFromUrl(url, name, currBoardId, itemId) {
  ...
  // processData(items.slice(1),boardId).catch((err) => {
