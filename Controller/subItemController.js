...
const SubItemController = async (req, res) => {
  try {
    ...
    console.log(jobCode, "Constructed jobCode");
...
const foundedSubitems=await getSubItems(itemId.create_item.id);
const existingQuantity=await getColumnValue(subitemId,"numeric_mkw87bs1")
    ...
    for (const linked of linkedItems) {
      ...
      let subId=foundedSubitems.find((subitem)=>subitem.name===subItemName)?.id;
      console.log(subId, "subId after check");
      if (subId) {
        // Update quantity if subitem exists
        const currentQuantity = await getProcurementSubitemDetails(
          subId,
          subQuantityColumnId
        );
        console.log(subitemId,"SUBITEMIDDDD")
        console.log(currentQuantity, "currentQuantity");
        const updatedQuantity = JSON.parse(currentQuantity) || 0 + JSON.parse(existingQuantity) || 0;
        await UpdateSubitemQuantity(
          subitemBoardId,
          subId,
          updatedQuantity,
          subQuantityColumnId,
        ...
        console.log(
          `Updated quantity for subitem ${subItemName} to ${updatedQuantity}`
        );
      } else {
